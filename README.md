...
| [07 — Cross-Version Opaque Recovery](experiments/07-cross-version-opaque-recovery-challenge/) | Can the floor stay safe and useful when an opaque evaluator changes source and a later evaluator source is unavailable? | The version-aware candidate resolved 154/168 decisions, emitted zero wrong resolved outputs, kept 14 decisions explicitly unresolved, and used 41 charged operations versus 168 full-oracle executions. |
...
cd ../07-cross-version-opaque-recovery-challenge
python3 -m unittest discover -s tests -v
